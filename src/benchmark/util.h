...
[[maybe_unused]] static uint64_t u64Rand (const uint64_t& min, const uint64_t& max) {
...
public:
    RandomKeyTrace (size_t count, bool is_seq = false, bool prepared_trace = true) {
        if (prepared_trace) {
            ...
            if (!is_seq) {
                ...
                std::uniform_int_distribution<uint64_t> uniform_dist(1, count);
                for (size_t i = 0; i < count; i++) {
                    keys_[i] = uniform_dist(generator);
                }
            } else {
                for (size_t i = 0; i < count; i++) {
                    keys_[i] = i + 1;
    ...
    void CreateReadWorkloadWithZipfianDistribution (uint64_t read_dataset_length,
        ...
        fprintf(stdout, "[YCSB] Zipfian constant %2.2f\n", distrib_param);
        ...
        fprintf(stdout, "[YCSB Data Distribution] 20 %2.3f 40 %2.3f 60 %2.3f 80 %2.3f 100 %2.3f\n",
              count20 * 1.0 / count100, count40 * 1.0 / count100, count60 * 1.0 / count100,
              count80 * 1.0 / count100, count100 * 1.0 / count100);
        fprintf(stdout, "[YCSB] Unique number for generated workload %lu\n", umap.size ());
    }
    ...
    void CreateReadWorkloadWithLocalSequential4Workloads (uint64_t read_dataset_length,
        ...
        fprintf(stdout, "[WorkloadShifting] Zipfian constant %2.2f KeySpace %lu nSegments %lu\n", distrib_param,
        ...
        // }
        // fprintf(stdout, "[WorkloadShifting Data Distribution] 20 %2.3f 40 %2.3f 60 %2.3f 80 %2.3f 100 %2.3f\n",
        //       count20 * 1.0 / count100, count40 * 1.0 / count100, count60 * 1.0 / count100,
        //       count80 * 1.0 / count100, count100 * 1.0 / count100);
        // fprintf(stdout, "[WorkloadShifting] Unique number for generated workload %lu\n", umap.size ());

        // fprintf(stdout, "[WorkloadShifting] Prepared read_i_");
        // for (size_t j = 0; j < temp_read_dataset_length; j++) {
        //     uint64_t start =
        ...
        //     }
        // }
        // fprintf(stdout, "[WorkloadShifting] Prepared whole read workload");

        // Randomize read_i_
        // std::random_shuffle (read_i_.begin (), read_i_.end ());

        // fprintf(stdout, "temp_read_dataset_length %llu", temp_read_dataset_length);
        // for (size_t j = 0; j < temp_read_dataset_length; j++) {
        //     uint64_t start =
        //         (segmentIndex[read_i_[j] - 1]) * segment_length;  // start position of key vector
        //     for (size_t k = 0; k < segment_length; k++) {
        ...
        fprintf(stdout, "Phase 1 prepared");
        // fprintf(stdout, "phase_length %lu temp %llu read_keys_distribution_v1 %lu", phase_length, temp.size(), read_keys_distribution_v1.size());
        ...
        fprintf(stdout, "keys_ size %lu temp %lu", keys_.size(), temp.size());
        ...
        fprintf(stdout, "Phase 2 prepared");
        temp.clear ();
        // temp.insert (temp.end (), keys_.begin () + 3 * phase_length, keys_.end ());
        // // Shift phases 1, 2, 3 to the right
        // std::move_backward (keys_.begin (), keys_.begin () + 3 * phase_length, keys_.end ());
        ...
        fprintf(stdout, "Phase 3 prepared");
        temp.clear ();
        // temp.insert (temp.end (), keys_.begin () + 3 * phase_length, keys_.end ());
        // // Shift phases 1, 2, 3 to the right
        // std::move_backward (keys_.begin (), keys_.begin () + 3 * phase_length, keys_.end ());
        ...
        fprintf(stdout, "Phase 4 prepared");
    ...
    void CreateReadWorkloadWithSegments (uint64_t read_dataset_length, uint64_t segment_length,
        ...
        if (is_sparse_hot_spot) {
            fprintf(stdout, "Make the sparse hot spot");
        ...
        std::mt19937 generator(rd());
        zipfian_int_distribution<int> distribution (1, nSegments, distrib_param);
        fprintf(stdout, "[YCSB] Zipfian constant %2.2f KeySpace %lu nSegments %lu\n", distrib_param,
              keyspace_length, nSegments);
        read_keys_.resize (read_dataset_length);
        read_i_.resize (temp_read_dataset_length);
    ...
    RangeIterator write_iterate_between (size_t start, size_t end) {
        return RangeIterator (&read_keys_, start, end);
    }
